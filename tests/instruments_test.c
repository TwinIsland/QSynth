...
void play_full_song(Synthesizer *synth)
{
    ...
    for (int chord = 0; chord < 4; chord++)
    {
        ...
        NoteCfg bass_cfg = {
            .amplitude = 0.6f, .duration_ms = 2000, .midi_note = chord_progression[chord] - 24, .pan = 0.2f, .velocity = 0.8f};
        synth_play_note(synth, INST_WARM_BASS, NOTE_CONTROL_DURATION, &bass_cfg);

        NoteCfg pad_cfg = {
            .amplitude = 0.4f, .duration_ms = 2000, .midi_note = chord_progression[chord], .pan = 0.5f, .velocity = 0.6f};
        synth_play_note(synth, INST_ETHEREAL_PAD, NOTE_CONTROL_DURATION, &pad_cfg);

        NoteCfg lead_cfg = {
            .amplitude = 0.5f, .duration_ms = 1500, .midi_note = chord_progression[chord] + 12, .pan = 0.8f, .velocity = 0.7f};
